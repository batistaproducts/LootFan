...
class DbService {
  ...
  async getFanTransactions(fanId: string): Promise<Transaction[]> {
    ...
    return data
        .filter((t: any) => {
            // Fallback para filtrar compra de créditos
            return t.prize_won && !t.prize_won.startsWith('Pacote de') && t.transaction_type !== 'SUBSCRIPTION';
        })
        .map((t: any) => ({
            ...this.mapTransaction(t),
            fanName: 'Você',
  ...
  async getCreatorAnalytics(creatorId: string): Promise<AnalyticsData> {
      // Reutiliza método getTransactions para garantir filtro por Creator correto e dados completos
      const transactions = await this.getTransactions(creatorId);
      ...
      transactions.forEach(t => {
          // Group by Fan
          const fanId = t.fanId || 'unknown'; 
          if (!fanGroups[fanId]) {
              fanGroups[fanId] = { count: 0, totalSpent: 0, name: t.fanName || 'Anônimo', lastDate: t.date };
          }
          fanGroups[fanId].count += 1;
          fanGroups[fanId].totalSpent += t.amountPaid;
          if (new Date(t.date) > new Date(fanGroups[fanId].lastDate)) {
              fanGroups[fanId].lastDate = t.date;
          }
          ...
          monthlyData[monthKey].revenue += t.amountPaid;
