...
function App() {
  ...
  const handleNavigation = (view: string) => {
      ...
      // Rotas do Fã
      if (view === 'fan-profile' || view === 'fan-prizes' || view === 'fan-support' || view === 'fan-payments') {
         setCurrentView(view);
         return;
      }
        ...
        <Suspense fallback={
            ...
            {(() => {
                if (currentView === 'fan-view' && viewCampaignSlug) {
                    return <FanView campaignSlug={viewCampaignSlug} user={currentUser} onLogin={() => handleOpenAuth(UserRole.FAN)} mode="real" />;
                }
                ...
                if (currentUser?.role === UserRole.FAN) {
                    ...
                    if (currentView === 'fan-payments') return <FanPayments user={currentUser} onNavigateToCampaign={(slug) => navigateToCampaign('dummy', slug)} />; // Dummy username, backend handles lookup
